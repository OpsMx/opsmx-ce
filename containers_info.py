'''
Author: OpsMx
Description: Retrives the docker containers details(Triggred by docker events)
'''
import os
import time
import json
...
try:
    ...
    print "Please set the KEY in environmetal variables/n"
    raise ValueError

docker_daemon_port=4342
...
def swarm_events():
    try:
        cli=docker.Client("0.0.0.0:{}".format(docker_daemon_port))
        events = cli.events(decode=True)
    except:
        print "Docker process binded on 4342 port? Please check! Exiting..."
        sys.exit()
    print "Listening Docker events on the port: {}".format(docker_daemon_port)
    for event in events:
        try:
